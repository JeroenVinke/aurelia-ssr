...
function getAssets(stats, suffix) {
    ...
    var outputDir = path.relative(path.resolve(__dirname, '..'), stats.compilation.outputOptions.path);
    var matching = assets.filter(function (name) { return name.endsWith(suffix); });
    return matching.map(function (name) { return outputDir + "/" + name; });
}
